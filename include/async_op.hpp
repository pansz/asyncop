...
public:
    ...
    auto then(F&& f) -> AsyncOp<unwrap_async_op_t<typename std::invoke_result<F, T>::type>> {
        ...
        m_promise->success_cb = [f = std::forward<F>(f), next_state, op_id = m_promise->op_id](T val) mutable {
        ...
        m_promise->error_cb = [f = std::forward<F>(f), next_state, op_id](ErrorCode err) mutable {
            ...
            try {
                if constexpr (is_async_op_v<InvokeResult>) {
                    ...
                    recovery_op.then([next_state](T value) mutable {
    ...
    auto next(SuccessF&& success_handler, ErrorF&& error_handler) -> AsyncOp<unwrap_async_op_t<typename std::invoke_result<SuccessF, T>::type>> {
        ...
        if (m_promise->success_cb) {
            spdlog::warn("AsyncOp[{}] next() called but success callback already set", id());
        }
        if (m_promise->error_cb) {
            spdlog::warn("AsyncOp[{}] next() called but error callback already set", id());
        }
        
        // Set success handler
        m_promise->success_cb = [success_handler = std::forward<SuccessF>(success_handler), 
                            next_state, op_id](T val) mutable {
            ...
            try {
                if constexpr (is_async_op_v<SuccessInvokeResult>) {
                    ...
                    result_op.then([next_state](RetType value) mutable {
                        next_state->resolveWith(std::move(value));
                    }).onError([next_state](ErrorCode e) mutable {
                        next_state->rejectWith(e);
                    });
        ...
        m_promise->error_cb = [error_handler = std::forward<ErrorF>(error_handler),
            ...
            try {
                ...
                // Re-thrown ErrorCode
                spdlog::debug("AsyncOp[{}] next() error handler re-threw error {}", 
                            op_id, e);
                next_state->rejectWith(e);
            } catch (const std::exception& e) {
    ...
    AsyncOp<T> finally(F&& cleanup_fn) {
        ...
        m_promise->success_cb = [cleanup, cleanup_done, result_state, op_id = m_promise->op_id](T val) mutable {
